...
namespace Parser
{
    ...
    class Arms
    {
		...
		public void startBrowser()
		{
			...
			browser.Navigate().GoToUrl("https://dungeon.su/articles/inventory/96-arms/");
		}
		...
		public async Task writingToFileAsync(List<string> str)
		{
			...
			// сохранение данных
			using (FileStream fs = new FileStream("Arms.json", FileMode.OpenOrCreate))
			{
				//записывает как UTF-8, но отображаются как ASCII символы
				await JsonSerializer.SerializeAsync<List<string>>(fs, str, options);
				Console.WriteLine("Data has been saved to file Arms.json");
			}
		}
	}
}
